    ...
    return new Promise((resolve, reject) => {
        usersDb.findOne({ username }, (err, user) => {
            if (user) {
                ...
                usersDb.insert({ username, password: hashedPassword, defaultCity }, (err, newDoc) => {
                    if (err) {
...
async function main() {
    try {
        ...
        // Retrieve and display request history
        ...
        if (history.length > 0) {
            ...
            if (useHistory && history[parseInt(useHistory) - 1]) {
                ...
                const forecast = await fetchWeather(selectedRequest.latitude, selectedRequest.longitude, selectedRequest.days);
                if (forecast) displayWeather(user.username, forecast);
                logRequest(user.username, selectedRequest.latitude, selectedRequest.longitude, selectedRequest.days);
                rl.close();
                return;
            }
        }

        // Enter new location if history is not used
