...
// Register a new user with optional home city
async function registerUser() {
    const username = await getUserInput("Enter a username: ");
    const password = await getUserInput("Enter a password: ");
    const homeCity = await getUserInput("Enter your home city (or leave blank): ");

    let homeLatitude = null, homeLongitude = null;
    if (homeCity) {
        homeLatitude = await getUserInput(`Enter latitude for ${homeCity}: `);
        homeLongitude = await getUserInput(`Enter longitude for ${homeCity}: `);
    }
...
// Set home city with latitude & longitude
async function setHomeCity(username) {
    const homeCity = await getUserInput("Enter your home city name: ");
    const homeLatitude = await getUserInput(`Enter latitude for ${homeCity}: `);
    const homeLongitude = await getUserInput(`Enter longitude for ${homeCity}: `);

    return new Promise((resolve, reject) => {
        usersDb.update({ username }, { $set: { homeCity, homeLatitude, homeLongitude } }, {}, err => {
            if (err) {
                console.error("Error updating home city:", err);
                reject(err);
            } else {
                console.log("Home city updated successfully!");
                resolve();
            }
        });
    });
...
// Save request history (Now includes city name)
function logRequest(username, city, latitude, longitude, days) {
    const timestamp = new Date().toISOString();
    historyDb.insert({ username, city, latitude, longitude, days, timestamp }, err => {
...
async function main() {
    try {
        ...
        let latitude, longitude, days, city;
        
        console.log("\nYour request history:");
        let historyOptions = [];
        
        // If user has a home city, set it as choice 0
        if (user.homeCity) {
            console.log(`0. Use your home city: ${user.homeCity} (Lat: ${user.homeLatitude}, Lon: ${user.homeLongitude})`);
            historyOptions.push({ city: user.homeCity, latitude: user.homeLatitude, longitude: user.homeLongitude, days: 3 });
        }

        history.forEach((req, index) => {
            console.log(`${index + 1}. ${req.city} (Lat: ${req.latitude}, Lon: ${req.longitude}, Days: ${req.days}, Date: ${req.timestamp})`);
        ...
        if (!latitude || !longitude || !days) {
            city = await getUserInput("Enter the city name: ");
        ...
        logRequest(user.username, city, latitude, longitude, days);
