...
const logDb = new Datastore({ filename: "weather-logs.db", autoload: true });
...
async function logWeatherRequest(username, latitude, longitude, days) {
    const logEntry = { username, latitude, longitude, days, timestamp: new Date() };
    logDb.insert(logEntry, (err) => {
        if (err) {
            console.error("Error logging weather request:", err);
        }
    });
}
...
async function main() {
    try {
        ...
        await logWeatherRequest(user.username, latitude, longitude, days);
